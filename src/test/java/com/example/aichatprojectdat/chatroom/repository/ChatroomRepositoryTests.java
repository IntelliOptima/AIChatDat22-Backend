...
public class ChatroomRepositoryTests extends AbstractIntegrationTest {
    ...
    void whenChatroomCreated_ChatroomSavedInDB_AndUserRelationCreated_ReturnChatroomAndRelation() {
        ...
        Mono<Tuple2<Chatroom, ChatroomUsersRelation>> resultMono = chatroomService.create(Chatroom.of(user.id()))
                .flatMap(chatroom ->
                        chatRoomUsersRelationService.create(ChatroomUsersRelation.of(user.id(), chatroom.id()))
        ...
        StepVerifier.create(resultMono)
                .consumeNextWith(tuple -> {
                    ...
                    assertTrue(chatroom.createdDate().isBefore(Instant.now()));
                    assertEquals(chatroom.chatroomUserCreatorId(), user.id());

                    // Assertions for ChatroomUsersRelation
                    assertNotNull(relation);
                    assertEquals(user.id(), relation.userId());
                    assertEquals(chatroom.id(), relation.chatroomId());
                })
                .verifyComplete();
    }
    ...
    void whenChatroomFetchedFromDb_AddUsersToChatroomAndStoreInDB_ReturnChatroomUsersRelation() {
        ...
        Chatroom chatroom = chatroomService.create(Chatroom.of(users.get(0).id())).block();
        ...
        Mono<Tuple2<Chatroom, List<ChatroomUsersRelation>>> resultMono = chatroomService.findById(chatroomId)
                .flatMap(existingChatroom ->
                        Flux.fromIterable(users)
                                .flatMap(user -> chatRoomUsersRelationService.create(ChatroomUsersRelation.of(existingChatroom.id(), user.id())))
        ...
        StepVerifier.create(resultMono)
                .consumeNextWith(tuple -> {
                    ...
                    assertEquals(foundChatroom.id(), chatroomId);
